$(document).ready(function() {
  ...
  var apiRoot = 'https://tasks-render44.onrender.com/v1/tasks';
 ...
 function handleTaskSubmitRequest(event) {
  ...
  $.ajax({
    ...
    processData: false,
    contentType: "application/json; charset=utf-8",
    // dataType: 'json', <-- TĘ LINIĘ USUŃ LUB ZAKOMENTUJ
    data: JSON.stringify({
      title: taskTitle,
      content: taskContent
    }),
    complete: function(data) {
      if(data.status === 200 || data.status === 201) { // dodaj status 201 na wszelki wypadek
        getAllTasks();
      }
    }
  });
